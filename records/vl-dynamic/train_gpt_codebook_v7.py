...
def quantize_state_dict_codebook(state_dict, codebook_size=256, use_codebook=True):
    """
    ...
    for name, tensor in state_dict.items():
        ...
        # Con untied: cae al INT6 SDClip igual que el resto (MSE~0, roundtrip limpio)
...
def main():
    ...
    while True:
        ...
        for ms in range(grad_accum_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                loss = model(x, y)
            # Repulsión Volumétrica (solo si VL_LAMBDA > 0)
            if args.vl_lambda > 0:
    ...
    # Usar codebook solo con tied embeddings (MSE bajo)
    # Con untied: SDClip INT6 en tok_emb para roundtrip limpio
    use_codebook = args.tie_embeddings
    quant_obj, quant_stats = quantize_state_dict_codebook(
        base_model.state_dict(), codebook_size=args.codebook_size, use_codebook=use_codebook)
