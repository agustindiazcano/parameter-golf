"""
MODIFICACIÓN V8: Cuantización Adaptativa por Varianza de Activación
- Durante entrenamiento: embedding normal en bfloat16
- En serialización: cuantización adaptativa guiada por varianza de activaciones
- Filas con alta varianza de activación → menos clipping → más precisión
- Filas con baja varianza → más compresión agresiva
- Con untied: tok_emb → SDClip adaptativo, lm_head → SDClip adaptativo
- Con tied: tok_emb → codebook K-means (MSE bajo garantizado)
- Fix roundtrip: recompilación post-deserialización

Autores: Agustín Díaz-Cano (UTN, ORCID: 0009-0001-4336-490X)
"""
...
class Hyperparameters:
    ...
    # Volumetric Repulsion (opcional, default OFF)
    ...
    calib_batches = int(os.environ.get("CALIB_BATCHES", 8))  # batches de calibración
...
# ==============================================================================
# CODEBOOK K-MEANS (solo para tied embeddings)
# ==============================================================================
...
# Inspirada en GPTQ pero usando varianza de activaciones como proxy del Hessiano
# Contribución original: Díaz-Cano (2026)
# ==============================================================================
...
def collect_activation_stats(model, train_loader, device, n_batches=8, seq_len=1024,
    ...
    """
    Recolecta varianza de activaciones de entrada a cada capa lineal.
    Usa forward hooks — sin gradientes, muy rápido (~5s).
    Retorna dict: {layer_name -> std_per_row (n_rows,)}
    """
    activation_stats = {}
    hooks = []
    ...
    def make_hook(name):
        def hook(module, inp, out):
            x = inp[0].detach().float()  # (batch, seq, dim) o (N, dim)
            x_flat = x.reshape(-1, x.shape[-1])  # (N, dim)
            # std de activaciones por columna de entrada → importancia por fila del peso
            col_std = x_flat.std(dim=0)  # (dim_in,)
            # Para matriz W (out, in): varianza por fila = suma ponderada por col_std
            w = module.weight.float()  # (out, in)
            row_importance = (w.abs() * col_std.unsqueeze(0)).mean(dim=1)  # (out,)
    ...
    # Registrar hooks en todas las CastedLinear con ndim==2 y numel > 65536
...
def quantize_float_tensor_adaptive(t, k_base=12.85, k_min=6.0, k_max=32.0,
    ...
    """
    Cuantización adaptativa: k = k_base * scale_por_importancia
    Filas más importantes → k más grande → menos clipping → más precisión
    Filas menos importantes → k más pequeño → más compresión

    Sin row_importance: equivalente a SDClip estándar con k=k_base
    """
    t32 = t.float()
    ...
    if t32.ndim == 2:
        if row_importance is not None and row_importance.numel() == t32.shape[0]:
            # Escalar k por importancia relativa de cada fila
            imp = row_importance.to(t32.device).float()
            imp_norm = imp / (imp.mean().clamp_min(1e-7))  # normalizar al promedio
            k_per_row = (k_base * imp_norm).clamp(k_min, k_max)  # (n_rows,)
            std_dev = t32.std(dim=1).clamp_min(1e-7)  # (n_rows,)
            clip_abs = (k_per_row * std_dev).unsqueeze(1)  # (n_rows, 1)
        else:
            # SDClip estándar
...
def quantize_state_dict_adaptive(state_dict, activation_stats, codebook_size=256,
    ...
    """
    Cuantización adaptativa:
    - tok_emb: codebook K-means (si tied) o SDClip adaptativo (si untied)
    - Matrices grandes: SDClip adaptativo guiado por varianza de activaciones
    - Tensores pequeños: float16 passthrough
    """
    ...
    for name, tensor in state_dict.items():
        ...
        # CODEBOOK para tok_emb — solo con tied embeddings
        ...
        # Buscar importancia de activaciones para esta capa
        row_importance = None
        ...
        q, s = quantize_float_tensor_adaptive(t, k_base=k_base, k_min=k_min, k_max=k_max,
...
def main():
    ...
    while True:
        ...
        for ms in range(grad_accum_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                loss = model(x, y)
            # Repulsión Volumétrica opcional (VL_LAMBDA=0 por default)
            if args.vl_lambda > 0:
    ...
    # Recolectar estadísticas de activación para cuantización adaptativa
    ...
    # Usar codebook solo con tied embeddings
    ...
    if master_process:
        with open("final_model.adaptive.ptz", "wb") as f: f.write(quant_blob)
        ...
        log0(f"Serialized adaptive+zlib: {quant_file_bytes} bytes "
             f"(ratio:{ratio:.2f}x) total:{quant_file_bytes + code_bytes} bytes")

    if distributed: dist.barrier()
    with open("final_model.adaptive.ptz", "rb") as f: blob_disk = f.read()
    quant_state = torch.load(io.BytesIO(zlib.decompress(blob_disk)),
    ...
    q_loss, q_bpb = eval_val(args, model, rank, world_size, device, grad_accum_steps,
                              val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
    torch.cuda.synchronize()
    log0(f"final_adaptive_roundtrip val_loss:{q_loss:.4f} val_bpb:{q_bpb:.4f} "
